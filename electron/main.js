...
function scheduleReminder(schedule) {
    ...
    const reminderTime = new Date(utcStartTime.getTime() - 3 * 60 * 1000);
...
app.whenReady().then(() => {
    ...
    ipcMain.handle('start-recording', () => {
        ...
        const audioFilePath = path.join(recordingsDir, `recording-${timestamp}.wav`)

        console.log('开始录音，文件路径：', audioFilePath);
